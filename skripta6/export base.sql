...
-- Dumping structure for view sql_stefan.customers_persons
-- Creating temporary table to overcome VIEW dependency errors
CREATE TABLE `customers_persons` (
	`CUST_FIRST_NAME` VARCHAR(35) NOT NULL COLLATE 'utf8_general_ci',
	`CUST_LAST_NAME` VARCHAR(35) NOT NULL COLLATE 'utf8_general_ci',
	`CUST_ADDRESS` VARCHAR(40) NOT NULL COLLATE 'utf8_general_ci',
	`CUST_CITY` VARCHAR(20) NOT NULL COLLATE 'utf8_general_ci'
) ENGINE=MyISAM;
...
-- Dumping structure for view sql_stefan.employees_dates
-- Creating temporary table to overcome VIEW dependency errors
CREATE TABLE `employees_dates` (
	`EMP_ID` VARCHAR(10) NOT NULL COLLATE 'utf8_general_ci',
	`DATE_HITE` DATE NOT NULL,
	`YEAR_HIRE` INT(4) NULL,
	`DAY_HIRE` VARCHAR(9) NULL COLLATE 'utf8mb4_general_ci',
	`EMP_YEARS` BIGINT(11) NULL
) ENGINE=MyISAM;
...
-- Dumping structure for view sql_stefan.pay_rates
-- Creating temporary table to overcome VIEW dependency errors
CREATE TABLE `pay_rates` (
	`EMP_ID` VARCHAR(10) NOT NULL COLLATE 'utf8_general_ci',
	`PAY_RATE` DECIMAL(6,2) NOT NULL,
	`NEW_PAY_RATE` DECIMAL(8,3) NOT NULL
) ENGINE=MyISAM;

-- Dumping structure for view sql_stefan.persons
-- Creating temporary table to overcome VIEW dependency errors
...
-- Dumping structure for view sql_stefan.price_list
-- Creating temporary table to overcome VIEW dependency errors
CREATE TABLE `price_list` (
	`PROD_NAME` VARCHAR(40) NOT NULL COLLATE 'utf8_general_ci',
	`PRICE` DECIMAL(6,2) NOT NULL,
	`TAX` DECIMAL(8,3) NOT NULL,
	`BRUTO_PRICE` DECIMAL(8,3) NOT NULL
) ENGINE=MyISAM;
...
-- Dumping data for table sql_stefan.products_tbl: ~12 rows (approximately)
...
-- Dumping structure for view sql_stefan.salaries
-- Creating temporary table to overcome VIEW dependency errors
CREATE TABLE `salaries` (
	`EMP_ID` VARCHAR(10) NOT NULL COLLATE 'utf8_general_ci',
	`POS` VARCHAR(20) NULL COLLATE 'utf8_general_ci',
	`SALARY` DECIMAL(8,2) NOT NULL,
...
DROP TABLE IF EXISTS `customers_persons`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customers_persons` AS select left(`customer_tbl`.`CUST_NAME`,(locate(' ',`customer_tbl`.`CUST_NAME`) - 1)) AS `CUST_FIRST_NAME`,right(`customer_tbl`.`CUST_NAME`,(length(`customer_tbl`.`CUST_NAME`) - locate(' ',`customer_tbl`.`CUST_NAME`))) AS `CUST_LAST_NAME`,`customer_tbl`.`CUST_ADDRESS` AS `CUST_ADDRESS`,`customer_tbl`.`CUST_CITY` AS `CUST_CITY` from `customer_tbl` where (`customer_tbl`.`CUST_GENDER` is not null);

-- Dumping structure for view sql_stefan.employees_dates
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `employees_dates`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `employees_dates` AS select `employee_pay_tbl`.`EMP_ID` AS `EMP_ID`,`employee_pay_tbl`.`DATE_HITE` AS `DATE_HITE`,year(`employee_pay_tbl`.`DATE_HITE`) AS `YEAR_HIRE`,dayname(`employee_pay_tbl`.`DATE_HITE`) AS `DAY_HIRE`,floor(((to_days(cast(now() as date)) - to_days(`employee_pay_tbl`.`DATE_HITE`)) / 365.25)) AS `EMP_YEARS` from `employee_pay_tbl`;

-- Dumping structure for view sql_stefan.pay_rates
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `pay_rates`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pay_rates` AS select `employee_pay_tbl`.`EMP_ID` AS `EMP_ID`,`employee_pay_tbl`.`PAY_RATE` AS `PAY_RATE`,(`employee_pay_tbl`.`PAY_RATE` * 1.1) AS `NEW_PAY_RATE` from `employee_pay_tbl` where (`employee_pay_tbl`.`PAY_RATE` is not null);

-- Dumping structure for view sql_stefan.persons
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `persons`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `persons` AS select `employee_tbl`.`LAST_NAME` AS `LAST_NAME`,`employee_tbl`.`FIRST_NAME` AS `FIRST_NAME`,'EMPLYEE' AS `STATUS` from `employee_tbl` union select `customers_persons`.`CUST_LAST_NAME` AS `CUST_LAST_NAME`,`customers_persons`.`CUST_FIRST_NAME` AS `CUST_FIRST_NAME`,'CUSTOMERS' AS `STATUS` from `customers_persons` order by `LAST_NAME` desc;

-- Dumping structure for view sql_stefan.price_list
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `price_list`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `price_list` AS select `products_tbl`.`PROD_NAME` AS `PROD_NAME`,`products_tbl`.`PRICE` AS `PRICE`,(`products_tbl`.`PRICE` * 0.2) AS `TAX`,(`products_tbl`.`PRICE` * 1.2) AS `BRUTO_PRICE` from `products_tbl`;

-- Dumping structure for view sql_stefan.salaries
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `salaries`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `salaries` AS select `employee_pay_tbl`.`EMP_ID` AS `EMP_ID`,`employee_pay_tbl`.`POS` AS `POS`,`employee_pay_tbl`.`SALARY` AS `SALARY`,`employee_pay_tbl`.`BONUS` AS `BONUS`,(`employee_pay_tbl`.`SALARY` + ifnull(`employee_pay_tbl`.`BONUS`,0)) AS `TOTAL` from `employee_pay_tbl`;
